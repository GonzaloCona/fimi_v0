...
public class FimiService {
	...
	public List<ObtenerGrafico> getDatos(int id){
		...
		try{
			...
			}else System.out.println("no tiene redes");
	...
	public int buscaEnHistorial(List<rs_usuario> lrs, DateHandler fecha){
		...
		try{
			...
			System.out.println(ex.getMessage());
			System.out.println("se cae la wea");
		}
		session.getTransaction().commit();	
		session.close();
		return count;
	 ...
	 public DateHandler sumarRestarDiasFecha( DateHandler fecha, int dias){
		   ...
		   try{    
		 	Calendar calendar = Calendar.getInstance();			
			   ...
			   System.out.println("CAGA AQUI en la fecha");
		   }
		   return null;
		...
		try{
			...
			if(p.size()==0){
				...
				if(id_red==2){
					...
					List<rs_usuario> lrs = (List<rs_usuario>) query.list();
					//System.out.println("pasa el select: "+lrs.size());
					if(lrs.size()==0){			
						key.setId_red_social(2);
						...
						salida.setCod_salida(0);
						salida.setDes_salida("inserto cod 2 y existia con sin_info");
	...
	public salidaStndr insertUser(String nom1,String nom2,String apell1,String apell2, String correo,String pass,String twit,String keyMovil)
	{
		...
		try{
			...
			//System.out.println("!!!!"+p.size());
			if(p.size()==0){
				System.out.println(" entra al crea usuario!!!!");
				...
				System.out.println("cagooo!");
				salida.setCod_salida(1);
				salida.setDes_salida("ya existe!!");
				
			}
		}catch(Exception ex){
			System.out.println("caga al principio !!");
			salida.setCod_salida(1);
			salida.setDes_salida(ex.getMessage());
	...
	public List<ObtenerHistorial> selectAll100(int id){
		...
		try{
			int max=100;
			int i=0;
			query1 = session.createQuery("from historial_usuario where id_usuario_red in "+idBus);//+" and id_red_social="+rsU.getId_red_social());
	...
	//MANTENEDOR NOTICIAS
	public salidaStndr insertNoticia(String seccion,String contenido,String titulo, String foto, int tipo,int estado,Date fecha){
		salidaStndr salida=new salidaStndr();
	...
	public List<ObtenerNoticia> selectNoticias(){
		...
		Query query1;
		try{
			query1 = session.createQuery("from noticias");
